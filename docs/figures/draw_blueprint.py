...
Writes heavyhex-blueprint.png and d5_fez_layout.json. The patches go where
heavyhex.patches.placement puts them for the saved fez_calibration.json, so
redraw after every `heavyhex calibrate`. Runs offline.
"""
...
from heavyhex.patches.placement import Calibration, Spot, best_spots, problems

HERE = Path(__file__).resolve().parent
FEZ_MAP = HERE / "fez_map.json"
CALIBRATION = HERE / "fez_calibration.json"
...
def draw_captions(
    fig: Figure, d3: HeavyHexLayout, d5: HeavyHexLayout, calibration: Calibration
) -> None:
    fig.suptitle(
...
def build_figure(
    device: DeviceMap, d3: HeavyHexLayout, d5: HeavyHexLayout, calibration: Calibration
) -> Figure:
    ...
    draw_legend(fig)
    draw_captions(fig, d3, d5, calibration)
    return fig


def write_manifest(
    layout: HeavyHexLayout, spot: Spot, calibration: Calibration, path: Path
) -> None:
    manifest = {
        "distance": layout.distance,
        "backend": calibration.backend,
        "calibration": calibration.calibrated,
        "first_data_position": list(spot.origin),
...
def main(argv: list[str] | None = None) -> None:
    ...
    fig = build_figure(device, d3, d5, calibration)
    png = args.out / PNG_NAME
    fig.savefig(png, dpi=160)
    plt.close(fig)
    write_manifest(d5, spots[5], calibration, args.out / MANIFEST_NAME)
    print(f"wrote {png}")
